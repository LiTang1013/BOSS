...
int WriteScript_corr(string leftread,string rightread,string contig,string output,int i){
    ...
    path=output.substr(0,pos)+"/mapping/BOSS_alignScript_corr.sh";
    cout<<"path:"<<path<<endl;
    ...
    location_out.open(openPath); 
        ...
        return 0;
    string out0=string("#!/bin/bash\n")+string("bowtie2-build ")+contig+string(" ")+output.substr(0,pos)+string("/mapping/index")+string("&&\n");
...
int WriteScript(string leftread,string rightread,string contig,string output,int i){
    ...
    string num=int_Tostring(i);
    pos=output.find_last_of('/');
    path=output.substr(0,pos)+"/mapping/BOSS_alignScript.sh";
    cout<<"path:"<<path<<endl;
    char * openPath = new char[50];
    openPath=(char*)path.c_str();
    location_out.open(openPath); 
...
int bashExe(string openpath){
    ...
    char line[300];
    FILE *fp;
    const char *sysCommand = cmd.data();                                       /*执行改错比对*/
...
int main(int argc, char *argv[]){
    ...
    string openpath,alignpath_corr,alignpath;
    ...
    for(int i = 0; i < libraryCount; i++){                                     
		if(i==0){
			...
			contig=openpath+"/mapping/correctionContig.fasta";
		}
		WriteScript_corr(argv[2+9*i],argv[2+9*i+1],contig,argv[argc-1],i);     
		bashExe(alignpath_corr);
		int mu=atoi(argv[2+9*i+3]);
		int sigma=mu/10;
		string muStr=int_Tostring(mu);
		string sigmaStr=int_Tostring(sigma);
		string paraStr_CfgFileName=fGetCfgFileName();
		string shell=string("python ")+paraStr_CfgFileName+string("mec.py -bam ")+openpath+string("/mapping/correction_sort0.bam -a 0.4 -b 0.5 -m ")+muStr+string(" -s ")+sigmaStr+string(" -i ")+argv[1]+string(" -o ")+openpath+string("/mapping/correctionContig.fasta");
		cout<<shell<<endl;
		shellExe(shell);
	...
	string input1,input2,num;
    for(int i = 0; i < libraryCount; i++){
		num=int_Tostring(i);
		input1=openpath+string("/mapping/correctionleft")+num+string(".bam");
		input2=openpath+string("/mapping/correctionright")+num+string(".bam");
        inputArg[i].bamFileName1 =(char*)input1.c_str();
        inputArg[i].bamFileName2 =(char*)input2.c_str();
    ...
    string input3=openpath+string("/mapping/correctionContig1.fasta");
